...
def nlpText(input_text, sia_tool):
    ...
    nlp_result = [sia_tool.polarity_scores(text)["pos"], sia_tool.polarity_scores(
        text)["neg"], sia_tool.polarity_scores(text)["compound"]]
        ...
        for j in input_city:
            if j.lower() in tweet_text.lower():
                if (str(i.id) not in dbrcity):
                    new_search = Search(_id=str(
                        i.id), city_name=j, nlppos=nlp_stat[0], nlpneg=nlp_stat[1], nlpemo=nlp_stat[2])
    ...
    while result_finish:
        try:
            if (state_start):
                ...
                result = client.search_recent_tweets(
                    query_rule, max_results=100, next_token=next_page)
                writeDb(result, dbrcity, city_set)
                if "next_token" in result[3]:
                    next_page = result[3]["next_token"]
                else:
...
def search_work(couch, input_keyword):
    city_set = ["Canberra", "Sydney", "Melbourne",
    ...
    city_count, city_pos, city_neg, city_emo = summaryView(
        'summary', input_keyword, dbrcity)
